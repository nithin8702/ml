                    ...
                    using (reader = ado.ExecDataReaderProc("RNDRegisteredUser_ReadByID", "RND", new object[] { param1 }))
                    {
                        if (reader.HasRows)
                        {
                            if (reader.Read())
                            {
                                ...
                                NUR.UserName = Convert.ToString(reader["UserName"]);
                                NUR.FirstName = Convert.ToString(reader["FirstName"]);
                                NUR.LastName = Convert.ToString(reader["LastName"]);
                                NUR.PermissionLevel = Convert.ToString(reader["PermissionLevel"]);
                            }
                        }
                    }
                }
